...
\boxed{
  \begin{minipage}{0.9\linewidth}
    \textbf{Énoncé du problème:} Estimer la loi des $S_i$ sachant que l'on peut mesurer expérimentalement $W_t$, que $N_t$ suit une loi de Poisson de paramètre $\lambda t$ et que l'on a l'expression \eqref{mod}. Trouver une mesure pertinente pour exprimer l'erreur entre la loi estimée et la loi <<~réelle~>>.
\end{minipage}}
...
\[\abs{f(x)-\hat{f}(x)}=O\pth{ \abs{\frac{2 d_k}{\ln^k\pth{\varepsilon}}}}\comment{quand $\varepsilon\to 0$}\]
}
...
\item Regardons maintenant le comportement de $A$ quand $\varepsilon\to 0$.
  ...
  \begin{align*}
    ...
    %& + \frac{2d_k}{\pth{k\ln\pth{\frac{1}{k}\pth{\frac{2\pi d_k}{\varepsilon}}^{1/k}}-k\ln\ln\pth{\frac{1}{k}\pth{\frac{2\pi d_k}{\varepsilon}}^{1/k}}+o(1)}^{k-1}}
  ...
  \begin{align*}
    \alpha(A_{\varepsilon})& \sim \frac{2 d_k}{\pth{k\ln\pth{\frac{1}{k}}+\ln\pth{2\pi d_k}+\abs{\ln\pth{\varepsilon}}}^k}% + \frac{2d_k}{\pth{k\ln\pth{\frac{1}{k}}+\ln\pth{\frac{2\pi d_k}{\varepsilon}}}^{k-1}} + o(1)\\
    %& = \frac{2 d_k}{\pth{k\ln\pth{\frac{1}{k}}+\ln\pth{\frac{2\pi d_k}{\varepsilon}}}^k} + o(1)
  \end{align*}
...
Dans cette partie, nous présentons une modélisation du problème par une EDP sur la densité de la loi de $\ln W_t$. Le but est de modéliser un processus limite en grande population. Toutefois, nous ne démontrons aucun résultat de convergence.% À partir de l'EDP, nous pourrons fournir une expression explicite de la loi de $\ln (1-S)$.
...
Faisons quelques transformations pour simplifier \eqref{edp_heur}. Comme $\int_{\Er}f(y)\de y = 1-\mu$:
...
On peut mesurer $u$ et on aimerait estimer $f$, en sachant que l'EDP \eqref{edp} est vérifiée: en quelque sorte, on connaît la solution mais on ne connaît pas le problème. On a la proposition suivante:
...
\begin{figure}[h]
  \begin{center}
    ...
    \includemedia[width=\linewidth, height=0.34\linewidth, 
    activate=pageopen,  passcontext,  transparent,  keepaspectratio,
    addresource=../Video/GrowthRates_simulationsInterval_VS_observations.mp4,  flashvars={source=../Video/GrowthRates_simulationsInterval_VS_observations.mp4
                  &autoPlay=true
                  &loop=true}]{}{VPlayer.swf}
  \end{center} 
