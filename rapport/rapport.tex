...
\begin{enumerate}
\item \textbf{microfluidic Mutation Accumulation ($\mu$MA) experiment} : suivi du taux d'élongation (proxy de la fitness) des bactéries sur de nombreuses générations en supprimant les effets de la sélection naturelle;
...
Dans cette expérience, on se place toujours dans la \emph{Mother Machine} et on observe par fluorescence (Fig. \ref{fig:MV}) l'apparition d'erreurs d'appariement entre paires de nucléotides dues à des erreurs de réplication de l'ADN (via l'insertion d'un rapporteur YFP-MutL). En prenant des mutants (\emph{MutH}, \emph{MutT}) dont le système de réparation est dysfonctionnel, on s'assure que toute erreur d'appariement donne une mutation. Les apparitions de taches fluorescentes correspondent donc bien à l'apparition d'une mutation.
...
Afin de tester expérimentalement l'hypothèse d'apparition poissonnienne des mutations, les auteur.ice.s ont évalué les intervalles de temps $\delta$ entre 2 apparitions de mutations dans l'expérience de MV, ces intervalles définissant un processus de Poisson si leurs longueurs suivent une distribution exponentielle et sont indépendantes deux à deux (et en particulier d'un interval $\delta_n$ au suivant $\delta_{n+1}$). Ces deux critères sont vérifiés, comme le montre la figure \ref{fig:iatime} (en corrigeant la distribution attendue pour des observations discontinues toutes les 4 minutes). Nous avons répliqué ces résultats dans le notebook \emph{I4\_Replication\_Dynamique-apparition-mutations}.

\begin{figure}[h]
  \begin{center}
    \vspace{3mm}
    \includegraphics[scale=0.5]{../Img/Interarrival_times.png}
  \end{center}  
  \caption{\label{fig:iatime}}
\end{figure}

De même, on peut vérifier avec la seconde expérience ($\mu$MA) que les mutations létales suivent une dynamique semblable. Cela n'est pas évident, a priori, si l'on imagine que certaines mutations ont plus de chances d'être létales si la cellule a déjà accumulé précédemment des mutation délétères dans des voies partiellement redondantes qui ne peuvent donc plus compenser une nouvelle perte de fonction. Empiriquement, on observe que les courbes de survie (Fig. \ref{fig:survival}) suivent une courbe exponentielle (voir la figure \label{fig:survival}). On remarque aussi qu'un phénomène de sénescence des cellules commence à émerger, bien visible à partir d'environ 40 heures chez la souche sauvage \emph{WT} qui a peu de mutations. Le phénomène de sénescence peut être corrigé en divisant par le taux de croissance des cellules WT (de taux d'apparition de mutations létales très faible). Seule la souche MF1 semble s'écarter légèrement d'une trajectoire exponentielle.
...
$$1 - s_\varepsilon \sim \frac{W'}{W}(1+\varepsilon'-\varepsilon)
= \prod_{i=0}^{N_{t'-t}}(1-s_i)(1+ \varepsilon'-\varepsilon)$$

Commençons par regarder les distributions de variations relatives des taux de croissance pour les différentes souches de bactéries (Fig. \ref{fig:VarRel}). Ces distributions de variations relatives de taux de croissance ne sont pas non plus normales, ni même symétrique si on superpose les 2 moitiés (verte et rouge).
...
\begin{figure}[!h]
  ...
  \end{center} 
  \caption{\label{fig:VarRel}}
\end{figure}

Plus le taux de mutation est faible (WT $<$ MutH $\simeq$ MutT $<$ WT) plus la distribution des sauts relatifs de croissance est étalée. La part du bruit devient plus importante que celle du signal.
...
Afin d'être les plus réalistes possibles, nos simulations suivent les étapes suivantes (voir Fig. \ref{fig:simulations}) :
\begin{itemize}
	\item Les taux de croissance sont initialisés avec ceux mesurés dans l'expérience (médiane des 10 premières valeurs);
	\item Une DFE est tirée dans un mélange de distributions au choix. Dans notre cas nous avons pris la distribution Beta inférée dans l'article plus un Dirac en 1 permettant d'incorporer des mutations létales;
	\item Les effets $s$ des mutations sur la fitness sont tirées dans cette DFE pour chacun des 1476 canaux de l'expérience et grand nombre de mutations;
	\item L'évolution du taux de croissance, mutation par mutation, est déduite des effets relatifs cumulés des mutations à partir des taux de croissance initiaux;
	\item Les durées entre 2 mutations sont tirées dans une loi exponentielle de paramètre le taux de mutation, qui peut être au choix constant dans le temps et entre les cellules ou décroissant avec le taux de croissance (le cycle cellulaire et la réplication étant ralentis);
	...
	\item Enfin, le résultats est renvoyé en rajoutant ou non un bruit Gaussien multiplicatif à chaque mesure, bruit ensuite moyenné sur chaque génération de bactérie comme indiqué dans l'article, en supposant que les bactéries se divisent dès qu'elles ont doublé de taille(bien qu'un modèle de division incrémental serait plus réaliste).
\end{itemize}
...
\begin{figure}[!h]
  \begin{center}
    ...
    addresource=../Video/GrowthRates_noisySimulationsInterval_VS_observations.mp4,  flashvars={source=../Video/GrowthRates_noisySimulationsInterval_VS_observations.mp4
                  ...
                  &loop=true}]{}{VPlayer.swf}
  \end{center} 
  \caption{\label{fig:animations}}
\end{figure}

Les animations présentées ici (Fig. \ref{fig:animations}) représentent l'évolution au cours du temps de la distribution des taux de croissance, en comparant les simulations aux données empirique, avec pour la seconde un intervalle des valeurs simulées sur 100 réplications. On observe que l'évolution de la distribution simulée est au début assez proche de celle observée, mais à la fin les deux divergent, probablement à cause de la DFE non adaptée (pas de mutations avantageuses, trop peu de mutations fortement délétères) et surtout le début de la sénescence des cellules après 40h.
...
En traçant les fonctions $t\mapsto E_n(t)$ (que l'on est capable de calculer à partir des observations de l'expérience $\mu$MA), on devrait obtenir des droites dont les pentes seront directement reliées aux moments de la DFE par un facteur $\lambda$. Dans l'annexe \ref{ann:slopes}, nous montrons que \cite{rob} a obtenu des droites et nous expliquons comment les obtenir; nous estimons également, avec cette méthode, les moments de la DFE.
...
\prop{Supposons que $f\in$. Soit $u\in\cdot$ une solution classique de \eqref{edp}. Pour tout $x\in\mathbb{R}$:
...
\[\ln\Esp{W'_{t_i}}=\Esp{W_{t_i}}+\Esp{1+\varepsilon_i}=\Esp{W_{t_i}}+\Esp{1+\varepsilon}\]
